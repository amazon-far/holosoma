...
from holosoma.utils.inference_helpers import (
    ...
    export_multi_motion_and_policy_as_onnx,
...
class FastSACAgent(BaseAlgo):
    ...
    def setup(self) -> None:
        ...
        self.algo_obs_dim_dict = algo_obs_dim_dict  # Store for later use

        # Note: Do NOT call _update_obs_dims_for_future_motion() here because
        # we add future_motion_obs separately to actor_obs_indices and critic_obs_indices.
        # Calling it would double-count the future_motion dimension.
        ...
        # Add future motion obs dimension
        try:
            motion_command = self.unwrapped_env.command_manager.get_state("motion_command")
            num_dofs = motion_command.motion.joint_pos.shape[1]
            self.future_motion_obs_dim = self.config.num_future_steps * num_dofs * 2
            logger.info(f"Adding future_motion_obs: {self.future_motion_obs_dim} dims ({self.config.num_future_steps} steps × {num_dofs} DOFs × 2)")
            # Add to actor_obs_indices so Actor gets correct input dimension
            self.actor_obs_indices["future_motion_obs"] = {
                "start": actor_obs_dim,
                "end": actor_obs_dim + self.future_motion_obs_dim,
                "size": self.future_motion_obs_dim,
            }
            actor_obs_dim += self.future_motion_obs_dim
        except Exception:
            self.future_motion_obs_dim = 0

        self.actor_obs_dim = actor_obs_dim
        logger.info(f"Final actor_obs_dim: {actor_obs_dim} (base: {actor_obs_dim - self.future_motion_obs_dim} + future_motion: {self.future_motion_obs_dim})")
        ...
        # Add future motion obs dimension to critic as well
        if self.future_motion_obs_dim > 0:
            self.critic_obs_indices["future_motion_obs"] = {
                "start": critic_obs_dim,
                "end": critic_obs_dim + self.future_motion_obs_dim,
                "size": self.future_motion_obs_dim,
            }
            critic_obs_dim += self.future_motion_obs_dim
        ...
        # Add future_motion_obs to obs_keys so Actor/Critic use correct input dimension
        if self.future_motion_obs_dim > 0:
            actor_mlp_obs_keys.append("future_motion_obs")
            critic_mlp_obs_keys.append("future_motion_obs")
    ...
    def _update_obs_dims_for_future_motion(self):
        """Update observation dimensions to account for future motion trajectory.
        
        Dynamically calculates future_motion_obs dimension from motion data
        and updates algo_obs_dim_dict for both actor and critic.
        """
        try:
            motion_command = self.unwrapped_env.command_manager.get_state("motion_command")
            num_dofs = motion_command.motion.joint_pos.shape[1]
            future_motion_obs_dim = self.config.num_future_steps * num_dofs * 2  # pos + vel for each step
            
            # Update dimensions
            self.algo_obs_dim_dict["actor_obs"] += future_motion_obs_dim
            self.algo_obs_dim_dict["critic_obs"] += future_motion_obs_dim
            
            logger.info(f"Updated observation dimensions:")
            logger.info(f"  actor_obs: {self.algo_obs_dim_dict['actor_obs']} (added {future_motion_obs_dim} for future motion)")
            logger.info(f"  critic_obs: {self.algo_obs_dim_dict['critic_obs']} (added {future_motion_obs_dim} for future motion)")
        except Exception as e:
            logger.warning(f"Could not update observation dims for future motion: {e}")

    def _get_future_motion_obs(self) -> torch.Tensor:
        """Get future motion trajectory (joint_pos and joint_vel for next N steps).
        
        Returns:
            Tensor of shape (num_envs, num_future_steps * (num_dofs * 2))
            containing [joint_pos_t+1, joint_vel_t+1, joint_pos_t+2, joint_vel_t+2, ...]
        """
        motion_command = self.unwrapped_env.command_manager.get_state("motion_command")
        current_time_steps = motion_command.time_steps  # shape: [num_envs]
        
        # Get num_dofs from joint_pos dimension
        num_dofs = motion_command.motion.joint_pos.shape[1]
        
        future_obs_list = []
        
        for step_offset in range(1, self.config.num_future_steps + 1):
            # Initialize tensor for this step
            future_joint_pos_step = torch.zeros(self.env.num_envs, num_dofs, device=self.device, dtype=torch.float32)
            future_joint_vel_step = torch.zeros(self.env.num_envs, num_dofs, device=self.device, dtype=torch.float32)
            
            # For multi-motion: fetch from correct motion for each environment
            if motion_command.is_multi_motion:
                for motion_id in range(motion_command.num_motions):
                    mask = motion_command.motion_ids == motion_id
                    if mask.any():
                        motion_obj = motion_command.motions_dict[motion_id]
                        motion_total_steps = motion_obj.time_step_total
                        
                        # Calculate future time steps (clamp to max motion length)
                        future_time_steps = torch.clamp(
                            current_time_steps[mask] + step_offset,
                            max=motion_total_steps - 1
                        )
                        
                        future_joint_pos_step[mask] = motion_obj.joint_pos[future_time_steps]
                        future_joint_vel_step[mask] = motion_obj.joint_vel[future_time_steps]
            else:
                # Single motion mode
                motion_total_steps = motion_command.motion.time_step_total
                future_time_steps = torch.clamp(
                    current_time_steps + step_offset,
                    max=motion_total_steps - 1
                )
                future_joint_pos_step = motion_command.motion.joint_pos[future_time_steps]
                future_joint_vel_step = motion_command.motion.joint_vel[future_time_steps]
            
            # Concatenate pos and vel for this step
            future_step_obs = torch.cat([future_joint_pos_step, future_joint_vel_step], dim=1)  # [num_envs, num_dofs*2]
    ...
    def learn(self) -> None:
        ...
        # Reset torch.compile cache to avoid stale graph issues
        torch._dynamo.reset()
        ...
        # Add future motion trajectory to observations
        future_motion_obs = self._get_future_motion_obs()
        logger.info(f"[DEBUG] Before concat: obs.shape={obs.shape}, future_motion_obs.shape={future_motion_obs.shape}")
        obs = torch.cat([obs, future_motion_obs], dim=1)
        critic_obs = torch.cat([critic_obs, future_motion_obs], dim=1)
        logger.info(f"[DEBUG] After concat: obs.shape={obs.shape}, critic_obs.shape={critic_obs.shape}")
        ...
        while self.global_step <= args.num_learning_iterations:
            ...
            with self.logging_helper.record_collection_time():
                ...
                # Add future motion trajectory to next observations
                future_motion_obs = self._get_future_motion_obs()
                next_obs = torch.cat([next_obs, future_motion_obs], dim=1)
                next_critic_obs = torch.cat([next_critic_obs, future_motion_obs], dim=1)

                # Compute 'true' next_obs and next_critic_obs for saving
                true_next_obs = torch.where(
                    truncations[:, None] > 0, 
                    torch.cat([infos["observations"]["final"]["actor_obs"], future_motion_obs], dim=1), 
                    next_obs
                )
                true_next_critic_obs = torch.where(
                    truncations[:, None] > 0,
                    torch.cat([infos["observations"]["final"]["critic_obs"], future_motion_obs], dim=1),
                    next_critic_obs,
                )
