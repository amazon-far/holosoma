...
class PPO(BaseAlgo):
    ...
    def setup(self):
        ...
        self._update_obs_dims_for_future_motion()
    ...
    def _get_future_motion_obs(self) -> torch.Tensor:
        ...
        Args:
            num_future_steps: Number of future steps to include (default: 3)
    ...
    def _rollout_step(self, obs_dict):
        with torch.inference_mode():
            for _ in range(self.config.num_steps_per_env):
                ...
                actor_obs = torch.cat([actor_obs, future_motion_obs], dim=1)
                ...
                if infos["time_outs"].any():
                    ...
                    # Add future motion trajectory to final observations
                    future_motion_obs = self._get_future_motion_obs()
                    final_critic_obs = torch.cat([final_critic_obs, future_motion_obs], dim=1)
            ...
            # Add future motion trajectory
            future_motion_obs = self._get_future_motion_obs()
            last_critic_obs = torch.cat([last_critic_obs, future_motion_obs], dim=1)
    ...
    def _compute_ppo_loss(self, minibatch: Minibatch):
        ...
        # Note: actor_obs and critic_obs already include future motion trajectory
        # (added during _rollout_step and stored in storage)
