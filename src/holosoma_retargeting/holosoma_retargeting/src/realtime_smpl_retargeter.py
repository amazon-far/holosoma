...
Salvaged from the FAR-pi v4 library-mode integration branch (same
``(24,7) -> (29,)`` contract used by the now-deleted
``holosoma_driver_node.py``). Moving it into holosoma core — rather than
rebuilding it on the FAR-pi side — is the architectural call: retargeting
belongs in the service (§"Non-Goals": exactly one process owns the SDK
and the retargeting pipeline that drives it).
"""
