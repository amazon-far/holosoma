    ...
    def run(self):
        ...
        try:
            for it in itertools.count():
                ...
                # Emitting this inside the control loop is work on the hot path, so
                # the interval is configurable and 0 turns it off. flush=True dropped:
                # loguru takes a bare kwarg as a format argument, so it never flushed
                # a stream.
