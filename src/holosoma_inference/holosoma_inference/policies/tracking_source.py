...
This is the service-side contract for an external process (e.g. the FAR-pi
``holosoma_driver``) to push SMPLH tracking targets into a running policy.
The channel is transport-agnostic: holosoma core defines the payload
schema and the injection protocol; the concrete transport (Unix socket,
shared memory, ...) lives on the consumer side.

Design notes — see ``docs/design/holosoma-integration.md`` §"Service API
Surface" in the FAR-pi repo (dev/asetapen/holosoma-service) for the full
...
* The payload is SMPLH-native (body + optional hand joints + gripper
  ...
  joint_vel) is retargeting and is a follow-up bead (fp-e59.A.2). Until
  that bridge lands, ``rl_inference`` logs the received payload but
  does not substitute it into ``motion_command_t``. This gives the
  FAR-pi side something to integration-test against (serialize → log
...
class TrackingPayload:
    ...
    Fields mirror the union of FAR-pi's ``holosoma_msgs`` topics:
    ``BodyTrackingState`` + ``GripperCommand`` + ``HolosomaControl``.
    Keep this schema stable; consumers on both sides of the service
    boundary are pickling / serializing to it.
    ...
    the service. The mode field is one of the ``HolosomaControl.mode``
    enum values (0=STOP, 1=TELEOP, 2=INFERENCE, 3=IDLE).
    """
    ...
    # HolosomaControl fields — carried per-tick so the service can respond
...
class TrackingSource(Protocol):
    ...
    implementation's concern. See fp-e59.B on the FAR-pi side for the
