...
class WholeBodyTrackingTargetPosePolicy(BasePolicy):
    def __init__(self, config: InferenceConfig):
        ...
        # Load TargetPose from JSON file
        self.TargetPose = self._load_target_pose(config.task.target_pose_path)

    def _load_target_pose(self, file_path: str):
        """
        Load the target pose from a JSON file.

        Args:
            file_path (str): Path to the JSON file containing the target pose.

        Returns:
            dict: Parsed target pose data.
        """
        try:
            with open(file_path, "r") as file:
                return json.load(file)
        except FileNotFoundError:
            raise FileNotFoundError(f"Target pose file not found: {file_path}")
    ...
    def setup_policy(self, model_path):
        ...
        self.motion_command_0 = np.concatenate([self.TargetPose["joint_pos"], self.TargetPose["joint_vel"]], axis=1)[0].copy()
        self.ref_quat_xyzw_0 = self.TargetPose["ref_quat_xyzw"][0].copy()      


        def policy_act(input_feed):
            output = self.onnx_policy_session.run(["actions", "joint_pos", "joint_vel", "ref_quat_xyzw"], input_feed)
            action = output[0]
            if time_step < len(self.TargetPose["joint_pos"]) - 1:
                time_step = np.int32(input_feed["time_step"][0, 0])
            else:
                time_step = len(self.TargetPose["joint_pos"]) - 1
            motion_command = np.concatenate([self.TargetPose["joint_pos"], self.TargetPose["joint_vel"]], axis=1)[time_step].copy()
            ref_quat_xyzw = self.TargetPose["ref_quat_xyzw"][time_step].copy() 
    ...
    def rl_inference(self, robot_state_data):
        ...
        policy_action, self.motion_command_t, self.ref_quat_xyzw_t = self.policy(input_feed, self.TargetPose)
