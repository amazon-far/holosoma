...
from holosoma_retargeting.data_conversion.convert_PHUMA_class import PHUMAConverter
...
class WholeBodyTrackingMultiMotionTargetPosePolicy(BasePolicy):
    def __init__(self, config: InferenceConfig):
        ...
        self.future_motion_obs_t = None
        self.motion_command_0 = None
        self.ref_quat_xyzw_0 = None
        self.future_motion_obs_0 = None
        ...
        if config.task.target_pose_path.endswith(".npz"):
            self.TargetPose = self._load_target_pose(config.task.target_pose_path)
        else:
            self.TargetPose = self.converter.convert(config.task.target_pose_path)
    ...
    def _load_target_pose(self, file_path: str) -> dict:
        """Load target pose from npz file and convert to TargetPose dictionary.

        Args:
            file_path: Path to npz file containing motion data.
                Expected keys: joint_pos, joint_vel, body_quat_w, body_names, fps

        Returns:
            Dictionary with keys:
                - joint_pos: (T, num_dofs) joint positions
                - joint_vel: (T, num_dofs) joint velocities
                - ref_quat_xyzw: (T, 4) reference body quaternion in xyzw format
                - fps: frame rate
                - total_frames: number of frames
        """
        if file_path is None:
            logger.warning("No target pose file specified, using empty TargetPose")
            return {}

        logger.info(f"Loading target pose from: {file_path}")

        with np.load(file_path, allow_pickle=True) as data:
            # Get required data
            joint_pos = data["joint_pos"].astype(np.float32)  # (T, 29)
            joint_vel = data["joint_vel"].astype(np.float32)  # (T, 29)
            body_quat_w = data["body_quat_w"].astype(np.float32)  # (T, num_bodies, 4) in xyzw
            fps = int(data["fps"][0]) if "fps" in data else 50

            # Get body names and find ref_body index
            body_names = data["body_names"].tolist()
            if isinstance(body_names[0], bytes):
                body_names = [name.decode("utf-8") for name in body_names]

            ref_body_index = body_names.index("torso_link")

            # Extract ref_body quaternion (already in xyzw format from convert_PHUMA_npy2train.py)
            ref_quat_xyzw = body_quat_w[:, ref_body_index, :]  # (T, 4)

            total_frames = joint_pos.shape[0]
            logger.info(f"  Loaded {total_frames} frames @ {fps} fps ({total_frames / fps:.2f}s)")
            logger.info(f"  joint_pos: {joint_pos.shape}, joint_vel: {joint_vel.shape}")
    ...
    def setup_policy(self, model_path):
        ...
        # Dynamically calculate future_motion_obs_dim from ONNX input shape
        # ONNX input: [obs] with shape (1, actor_obs_dim + future_motion_obs_dim)
        onnx_inputs = self.onnx_policy_session.get_inputs()
        obs_input = next((inp for inp in onnx_inputs if inp.name == "obs"), None)
        if obs_input is None:
            raise ValueError("ONNX model must have 'obs' input")
        obs_input_dim = obs_input.shape[1]
        actor_obs_dim = obs.shape[1]
        future_motion_obs_dim = obs_input_dim - actor_obs_dim
        
        # First ONNX run: use placeholder for future_motion_obs to get initial values
        future_motion_obs_placeholder = np.zeros((1, future_motion_obs_dim), dtype=np.float32)
        obs_with_future = np.concatenate([obs, future_motion_obs_placeholder], axis=1)
        input_feed = {"obs": obs_with_future, "time_step": time_step}
        outputs = self.onnx_policy_session.run(["joint_pos", "joint_vel", "ref_quat_xyzw", "future_motion_obs"], input_feed)
        ...
        self.future_motion_obs_t = outputs[3]

        # obs_with_future = np.concatenate([obs, self.future_motion_obs_t], axis=1)
        # input_feed = {"obs": obs_with_future, "time_step": time_step}
        ...
        # Initialize motion_command_0 and ref_quat_xyzw_0 with shape (1, N)
        self.motion_command_0 = np.concatenate(
            [self.TargetPose["joint_pos"][0:1], self.TargetPose["joint_vel"][0:1]], axis=1
        )  # (1, 58)
        self.ref_quat_xyzw_0 = self.TargetPose["ref_quat_xyzw"][0:1].copy()  # (1, 4)
        
        # Initialize future_motion_obs_t for frame 0
        total_frames = self.TargetPose["total_frames"]
        future_motions_init = []
        for i in range(1, 4):  # i = 1, 2, 3
            future_idx = min(i, total_frames - 1)
            future_motion = np.concatenate(
                [self.TargetPose["joint_pos"][future_idx:future_idx+1],
                 self.TargetPose["joint_vel"][future_idx:future_idx+1]],
                axis=1
            )  # (1, 58)
            future_motions_init.append(future_motion)
        self.future_motion_obs_0 = np.concatenate(future_motions_init, axis=1)  # (1, 174)
        
        # Initialize motion_command_t and ref_quat_xyzw_t
        self.motion_command_t = self.motion_command_0.copy()
        self.ref_quat_xyzw_t = self.ref_quat_xyzw_0.copy()
        self.future_motion_obs_t = self.future_motion_obs_0.copy()

        def policy_act(input_feed):
            output = self.onnx_policy_session.run(["actions"], input_feed)
            action = output[0]
            
            # Get current time_step
            time_step = int(input_feed["time_step"][0, 0])
            total_frames = self.TargetPose["total_frames"]
            
            # Clamp time_step to valid range
            time_step = min(time_step, total_frames - 1)
            
            # Get motion_command at current time_step - shape (1, 58)
            motion_command = np.concatenate(
                [self.TargetPose["joint_pos"][time_step:time_step+1],
                 self.TargetPose["joint_vel"][time_step:time_step+1]],
                axis=1
            )
            
            # Get ref_quat_xyzw at current time_step - shape (1, 4)
            ref_quat_xyzw = self.TargetPose["ref_quat_xyzw"][time_step:time_step+1]
            
            # Build future_motion_obs from time_step+1, time_step+2, time_step+3
            future_motions = []
            for i in range(1, 4):  # i = 1, 2, 3
                future_idx = min(time_step + i, total_frames - 1)
                future_motion = np.concatenate(
                    [self.TargetPose["joint_pos"][future_idx:future_idx+1],
                     self.TargetPose["joint_vel"][future_idx:future_idx+1]],
                    axis=1
                )  # (1, 58)
                future_motions.append(future_motion)
            
            # Concat all future motions: (1, 58) * 3 -> (1, 174)
            future_motion_obs = np.concatenate(future_motions, axis=1)

            return action, motion_command, ref_quat_xyzw, future_motion_obs
        ...
        state.update(
            {
                ...
                "future_motion_obs_0": self.future_motion_obs_0.copy(),
            }
        )
        return state
    ...
    def _restore_policy_state(self, state):
        ...
        self.future_motion_obs_0 = state["future_motion_obs_0"].copy()
        self.motion_clip_progressing = False
        self.motion_timestep = 0
        self.motion_start_timestep = None
    ...
    def rl_inference(self, robot_state_data):
        ...
        obs["actor_obs"] = np.concatenate([obs["actor_obs"], self.future_motion_obs_t], axis=1)
        input_feed = {"time_step": np.array([[self.motion_timestep]], dtype=np.float32), "obs": obs["actor_obs"]}
        
        policy_action, self.motion_command_t, self.ref_quat_xyzw_t, self.future_motion_obs_t = self.policy(input_feed)
