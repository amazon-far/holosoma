...
class WholeBodyTrackingMultiMotionPolicy(BasePolicy):
    ...
    def setup_policy(self, model_path):
        ...
        def policy_act(input_feed):
            output = self.onnx_policy_session.run(["actions", "joint_pos", "joint_vel", "ref_quat_xyzw", "future_motion_obs"], input_feed)
            action = output[0]
            motion_command = np.concatenate(output[1:3], axis=1)
            ref_quat_xyzw = output[3]
            future_motion_obs = output[4]
