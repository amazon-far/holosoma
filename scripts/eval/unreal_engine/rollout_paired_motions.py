"""Run paired student/teacher rollouts on a small filtered motion set.

Boots the sim once, sets up a multi-motion env on a directory containing N
hand-picked .npz files (one per env, pool_ordered + bind_terrain_tiles), and
runs two consecutive rollouts:

    Phase A: ``env.step({"actions": student_actions})`` for ~max_steps.
    Phase B: env.reset_all(), then ``env.step({"actions": teacher_actions})``.

For every env we record ``(simulator.robot_root_states[env, :3] -
scene.env_origins[env])`` so the trajectory lives in the same tile-local frame
as the motion's ``mesh_vertices`` (which the visualiser later renders as the
fixed terrain). One ``student_<name>.npy`` and one ``teacher_<name>.npy`` are
saved per motion, both clipped to the motion's actual length.

Usage:
    python scripts/eval/unreal_engine/rollout_paired_motions.py \
        --student_ckpt logs/.../model_40000.pt \
        --motion_dir /tmp/dagger_sweep/filtered_motions \
        --out_dir /tmp/dagger_sweep/rollouts
"""

from __future__ import annotations

import argparse
import dataclasses
import os
from pathlib import Path

import numpy as np
import torch
from loguru import logger

from holosoma.utils.eval_utils import (
    CheckpointConfig,
    init_eval_logging,
    load_saved_experiment_config,
)
from holosoma.utils.experiment_paths import get_experiment_dir, get_timestamp
from holosoma.utils.helpers import get_class
from holosoma.utils.sim_utils import close_simulation_app, setup_simulation_environment


def _override_motion_cfg(eval_cfg, motion_dir: str, n_motions: int):
    motion_config = eval_cfg.command.setup_terms["motion_command"].params["motion_config"]
    if isinstance(motion_config, dict):
        motion_config["motion_dir"] = motion_dir
        motion_config["max_motions"] = n_motions
        motion_config["pool_size"] = n_motions
    else:
        new_motion_config = dataclasses.replace(
            motion_config, motion_dir=motion_dir, max_motions=n_motions, pool_size=n_motions
        )
        eval_cfg.command.setup_terms["motion_command"].params["motion_config"] = new_motion_config

    terrain_term = eval_cfg.terrain.terrain_term
    new_terrain_term = dataclasses.replace(
        terrain_term, obj_dir=motion_dir, obj_max_tiles=n_motions
    )
    return dataclasses.replace(
        eval_cfg, terrain=dataclasses.replace(eval_cfg.terrain, terrain_term=new_terrain_term)
    )


def _setup_deterministic_batch(env, motion_cmd):
    """Force env_i → motion_i + tile_i + frame 0, no init pose noise.

    Mirrors ``SuccessRateCallback._setup_batch`` (`success_rate_callback.py:135`).
    Without this, ``MultiMotionCommand.reset`` randomly re-samples
    motion_indices on every env reset (including auto-resets mid-rollout),
    which breaks any attempt at fair student-vs-teacher comparison and
    desynchronises ``env_origin`` from the motion .npz the visualiser reads.
    """
    n = env.num_envs
    device = env.simulator.dof_pos.device

    motion_cmd.motion_indices[:] = torch.arange(n, device=device)
    motion_cmd.time_steps[:] = 0

    lib = motion_cmd.motion_library
    mi = motion_cmd.motion_indices
    ts = motion_cmd.time_steps

    # body 0 in the npz is the world placeholder; the actual root is body 0 of
    # the simulator's own root_states view, which is fed from
    # body_pos_w(mi, ts)[:, 0]. (Mirrors SuccessRateCallback exactly.)
    root_pos = lib.body_pos_w(mi, ts)[:, 0].clone()
    root_rot = lib.body_quat_w(mi, ts)[:, 0].clone()
    root_lin_vel = lib.body_lin_vel_w(mi, ts)[:, 0].clone()
    root_ang_vel = lib.body_ang_vel_w(mi, ts)[:, 0].clone()
    dof_pos = lib.joint_pos(mi, ts).clone()
    dof_vel = lib.joint_vel(mi, ts).clone()

    tile_grid = getattr(motion_cmd, "_tile_grid", None)
    if tile_grid is not None:
        row_idx = torch.zeros(n, dtype=torch.long, device=device)
        col_idx = mi
        new_origins = tile_grid[row_idx, col_idx]
        env.simulator.scene.env_origins[:] = new_origins
        # Body data is in motion-LOCAL frame; sim wants WORLD-frame poses.
        root_pos = root_pos + new_origins

    env.simulator.dof_pos[:] = dof_pos
    env.simulator.dof_vel[:] = dof_vel
    env.simulator.robot_root_states[:, :3] = root_pos
    env.simulator.robot_root_states[:, 3:7] = root_rot
    env.simulator.robot_root_states[:, 7:10] = root_lin_vel
    env.simulator.robot_root_states[:, 10:13] = root_ang_vel

    all_ids = torch.arange(n, device=device)
    env.simulator.set_actor_root_state_tensor_robots(all_ids, env.simulator.robot_root_states)
    env.simulator.set_dof_state_tensor_robots(all_ids, env.simulator.dof_state)

    env.episode_length_buf[:] = 0
    env.reset_buf[:] = 0
    env.time_out_buf[:] = 0


def _record_trajectories(env, num_envs, max_steps, action_fn, motion_lengths):
    """Run env deterministically (env_i bound to motion_i), recording per-env
    (root_pos_local, root_quat, dof_pos). Each env stops being recorded once
    it terminates or its motion ends; values past that frame are filled with
    the last valid value (visualiser will trim per motion length anyway)."""
    simulator = env.simulator
    obs_dict = env.reset_all()
    motion_cmd = env.command_manager.get_state("motion_command")
    _setup_deterministic_batch(env, motion_cmd)
    # After overriding state we must refresh the obs dict so the policy sees
    # the new pose.
    obs_dict = env.observation_manager.compute()
    device = simulator.dof_pos.device
    for k in obs_dict:
        obs_dict[k] = obs_dict[k].to(device)

    root_pos_buf = np.zeros((max_steps, num_envs, 3), dtype=np.float32)
    root_quat_buf = np.zeros((max_steps, num_envs, 4), dtype=np.float32)
    dof_pos_buf = np.zeros((max_steps, num_envs, simulator.dof_pos.shape[1]), dtype=np.float32)
    per_env_done_step = np.full(num_envs, max_steps, dtype=np.int64)

    env_done_mask = torch.zeros(num_envs, dtype=torch.bool, device=device)

    motion_lengths_t = torch.as_tensor(motion_lengths, device=device, dtype=torch.long)
    max_motion_len = int(motion_lengths_t.max().item())
    # Run a small slack past the longest motion so a slightly slow env can
    # still finish.
    horizon = min(max_steps, max_motion_len + 60)

    for step in range(horizon):
        root_state = simulator.robot_root_states[:, :].detach().cpu().numpy()
        env_origins = simulator.scene.env_origins.detach().cpu().numpy()
        # Tile-local frame so trajectory aligns with motion .npz mesh_vertices.
        root_pos_buf[step] = root_state[:, 0:3] - env_origins
        root_quat_buf[step] = root_state[:, 3:7]
        dof_pos_buf[step] = simulator.dof_pos.detach().cpu().numpy()

        with torch.inference_mode():
            actions = action_fn(obs_dict)
        obs_dict, _, dones, _ = env.step({"actions": actions})
        for k in obs_dict:
            obs_dict[k] = obs_dict[k].to(device)

        # Mark envs that just finished (terminated or motion ended).
        cur_ts = motion_cmd.time_steps
        motion_finished = cur_ts >= (motion_lengths_t - 1)
        dones_bool = dones.to(torch.bool).view(-1)
        finished = dones_bool | motion_finished
        newly_done = finished & ~env_done_mask
        if newly_done.any():
            for i in newly_done.nonzero(as_tuple=True)[0].tolist():
                per_env_done_step[i] = step + 1  # last valid recorded frame is `step`
        env_done_mask |= finished

        if env_done_mask.all():
            logger.info(f"All envs finished at step {step}")
            break

    actual_steps = step + 1
    return {
        "root_pos": root_pos_buf[:actual_steps],
        "root_quat": root_quat_buf[:actual_steps],
        "dof_pos": dof_pos_buf[:actual_steps],
        "motion_lengths": motion_lengths.copy(),
        "per_env_done_step": per_env_done_step,
    }


def main():
    init_eval_logging()
    parser = argparse.ArgumentParser()
    parser.add_argument("--student_ckpt", required=True)
    parser.add_argument("--motion_dir", required=True, help="Filtered motion dir (N .npz, pool_ordered).")
    parser.add_argument("--out_dir", required=True)
    parser.add_argument("--max_steps", type=int, default=1500)
    args = parser.parse_args()

    motion_dir = str(Path(args.motion_dir).expanduser().resolve())
    motion_files = sorted(p for p in Path(motion_dir).glob("*.npz"))
    if not motion_files:
        raise FileNotFoundError(f"No .npz files in {motion_dir}")
    n_motions = len(motion_files)
    logger.info(f"{n_motions} motions in {motion_dir}")

    out_dir = Path(args.out_dir).expanduser().resolve()
    out_dir.mkdir(parents=True, exist_ok=True)

    saved_cfg, saved_wandb_path = load_saved_experiment_config(
        CheckpointConfig(checkpoint=args.student_ckpt)
    )

    saved_cfg = dataclasses.replace(
        saved_cfg,
        eval_overrides=dataclasses.replace(
            saved_cfg.eval_overrides, num_envs=n_motions, headless=True,
        ),
    )
    eval_cfg = saved_cfg.get_eval_config()
    eval_cfg = _override_motion_cfg(eval_cfg, motion_dir, n_motions)

    env, device, simulation_app = setup_simulation_environment(eval_cfg)

    log_dir = get_experiment_dir(
        eval_cfg.logger, eval_cfg.training, get_timestamp(), task_name="rollout_paired"
    ).resolve()
    log_dir.mkdir(parents=True, exist_ok=True)

    algo_class = get_class(eval_cfg.algo._target_)
    algo = algo_class(
        device=device, env=env, config=eval_cfg.algo.config,
        log_dir=str(log_dir), multi_gpu_cfg=None,
    )
    algo.setup()  # builds student + loads teacher (saved policy_to_clone path)
    algo.attach_checkpoint_metadata(saved_cfg, saved_wandb_path)
    algo.load(args.student_ckpt)
    algo.actor.eval()
    if hasattr(algo, "teacher_actor") and algo.teacher_actor is not None:
        algo.teacher_actor.eval()
    else:
        raise RuntimeError("Teacher actor missing; rerun training with policy_to_clone set.")

    motion_cmd = env.command_manager.get_state("motion_command")
    motion_lengths = motion_cmd.motion_library.time_step_totals.detach().cpu().numpy()
    logger.info(f"Per-motion lengths: {motion_lengths.tolist()}")

    def student_action(obs_dict):
        state, _ = algo._build_student_state(obs_dict)
        return algo.actor.act_inference(state)

    def teacher_action(obs_dict):
        teacher_state = algo._build_teacher_state(obs_dict)
        return algo.teacher_actor.act_inference(teacher_state)

    try:
        env.set_is_evaluating()
        try:
            logger.info("=== Phase A: student rollout ===")
            student_traj = _record_trajectories(
                env, n_motions, args.max_steps, student_action, motion_lengths
            )
            logger.info("=== Phase B: teacher rollout ===")
            teacher_traj = _record_trajectories(
                env, n_motions, args.max_steps, teacher_action, motion_lengths
            )
        except Exception:
            import traceback
            logger.error("Rollout crashed:\n" + traceback.format_exc())
            raise

        for env_id, motion_path in enumerate(motion_files):
            motion_n = int(motion_lengths[env_id])
            base = motion_path.stem
            for label, traj in (("student", student_traj), ("teacher", teacher_traj)):
                end = min(
                    motion_n,
                    int(traj["per_env_done_step"][env_id]),
                    traj["root_pos"].shape[0],
                )
                end = max(end, 1)  # always save at least 1 frame
                rp = traj["root_pos"][:end, env_id]
                rq = traj["root_quat"][:end, env_id]
                dp = traj["dof_pos"][:end, env_id]
                out_path = out_dir / f"{label}_{base}.npy"
                np.save(
                    out_path,
                    {
                        "root_trans": rp,
                        "root_ori": rq,
                        "dof_pos": dp,
                        "fps": 50,
                    },
                )
                logger.info(f"Saved {out_path} ({end} frames; motion_len={motion_n})")

    finally:
        if simulation_app:
            close_simulation_app(simulation_app)


if __name__ == "__main__":
    main()
