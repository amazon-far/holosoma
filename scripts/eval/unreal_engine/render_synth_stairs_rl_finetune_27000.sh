#!/bin/bash
# Run the RL-finetuned student (model_27000) on the 100 synthetic stair tiles
# at h ∈ [0.10, 0.30] m, w ∈ [0.30, 0.80] m. Success criterion is "didn't
# fall AND reached the motion's goal x" — motion-tracking quality is ignored
# (synthetic motions hold the joints in standing pose, so the student can't
# possibly track the body trajectory anyway).
#
# Pipeline:
#   1. rollout_single_policy.py — deterministic single-pass rollout over all
#      100 motions, saves policy_<motion>.npy.
#   2. relabel_reach_target.py — labels each motion success/fail by
#      (reached ≥ 90% of goal_x) AND (final pelvis z within ±1m of motion's
#      final z) AND (rollout wasn't trimmed early).
#   3. Sample 10 success + 10 fail (or as many as exist) and render with
#      visualize.py — main = student rollout, ghost = motion .npz.
#   4. synth_stairs_analysis_reach.py — plots and CSV.
