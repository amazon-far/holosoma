...
# ----------------------------------------------------------------------------
# VideoMimic-style heightmap encoder variant.
#
# Architecture (matches VideoMimic rsl_rl/modules/actor_critic.py::
# FlattenThenEmbedMLPWithAttention):
#   height_map_obs (11 x W x 3 flattened)
#     -> Flatten
#     -> Linear(H*W*C, latent_dim=64)
...
#     -> MLP actor/critic
#
# Note: the RayCaster sensor currently produces a 17 x 11 grid (resolution=0.1
# over [1.6m, 1.0m]), so map_width=17, map_height=11. The "(11 x 11)" shape
# the videomimic reference uses is only a naming convention — the flatten +
# linear + gate pipeline is grid-shape-agnostic.
# ----------------------------------------------------------------------------
python src/holosoma/holosoma/train_agent.py \
    ...
    --logger.name=g1_stair_30depth_with_videomimic_heightmap_17x11 \
    ...
    --simulator.config.videomimic-height-map.num-channels 1 \
    ...
    --command.setup_terms.motion_command.params.motion_config.motion_file="holosoma/data/motions/g1_29dof/whole_body_tracking/unreal_engine/g1_stair_30depth_20260419.npz" \
...
# ----------------------------------------------------------------------------
# Previous (CNN + CrossAttention) heightmap invocation — kept for reference.
